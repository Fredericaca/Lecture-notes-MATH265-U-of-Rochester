...
\begin{proof}
    Let \(\epsilon>0\). Apply Archimedean's property to \(\frac{1}{\epsilon}\), then
    \begin{align*}
...
\begin{theorem}
    ...
    \begin{proof}
        By definition of convergent sequence, let \(\epsilon=1\):
...
\begin{theorem}[\textbf{Limit Laws}]
    ...
    \begin{proof}[Proof of Sum]:
        ...
        \(\forall \epsilon>0\),
        \[\exists N_1\in\N \st \forall n\ge N_1, \abs{x_n-x}<\frac{\epsilon}{2}\]
        \[\exists N_2\in\N \st \forall n\ge N_2, \abs{y_n-y}<\frac{\epsilon}{2}\]
    ...
    \begin{proof}[Proof of Product]:
        ...
        By definition of convergence, \(\forall \epsilon>0\):
        \[\exists N_1>0\st \forall n\ge N,\abs{x_n-x}<\frac{\epsilon}{2\abs{y}}\]
        \[\exists N_2>0\st \forall n\ge N,\abs{y_n-y}<\frac{\epsilon}{2M}\]
        ...
        \begin{align*}
            ...
            &<M\cdot \frac{\epsilon}{2M}+\abs{y}\cdot \frac{\epsilon}{2\abs{y}}=\epsilon
        \end{align*}
    \end{proof}
    ...
    \begin{proof}[Proof of Quotient]
        ...
        Thus \(\forall n>0\)
        \[\exists N_2>0\st\forall n\ge N_2, \abs{x_n-x}<\frac{\epsilon}{2GF}\]
        \[\exists N_3>0\st\forall n\ge N_3, \abs{y_n-y}<\frac{\epsilon}{2GF}\]
        Define \(N:=\max{\{N_1,N_2,N_3\}}\), then \(\exists N\in\N \st \forall n>N\),
        \[\abs{\frac{x_n}{y_n}-\frac{x}{y}}<F(G\cdot\frac{\epsilon}{2GF}+G\cdot\frac{\epsilon}{2GF})=\epsilon\]
...
\begin{theorem}
    ...
    \begin{proof}[Sketch]:
        Assume \(x<0\), choose \(\epsilon \le \abs{x}\), then \(\forall n > N, x_n<0\), which is a contradiction.
        \begin{center}
            \begin{tikzpicture}
                ...
                \draw (0,-0.5) node{0};
                \filldraw[black] (-6,0) circle (2pt);
                \draw (-6,-0.5) node{\(x+\epsilon\)};
                \filldraw[black] (-2,0) circle (2pt);
                \draw (-2,-0.5) node{\(x-\epsilon\)};
                \filldraw[black] (-4,0) circle (2pt);
                \draw (-4,-0.5) node{\(x\)};
            \end{tikzpicture}
        \end{center}
...
\begin{theorem}[\textbf{Squeeze Theorem}]
    ...
    \begin{proof}
        ...
        \[\abs{x_n-L}<\epsilon, \abs{z_n-L}<\epsilon\]
        \begin{align*}
            \Rightarrow&& -\epsilon<x_n-L&\le y_n-L\le z_n-L<\epsilon\\
            \Rightarrow&& \abs{y_n-L}&<\epsilon\\
        \end{align*}
...
\begin{theorem}
    ...
    \begin{proof} Let \(\forall \epsilon>0\)

        \highlight{Case1:} \(x=0\)

        \(\exists N\in\N \st \forall n\ge N\), \[\abs{x_n-0}<\epsilon^2\]
        \[\Rightarrow \abs{\sqrt{x}-0}<\epsilon\]

        \highlight{Case2:} \(x>0\)

        \(\exists N \st \forall n>N,\) \[\abs{x_n-x}<\sqrt{x}\cdot\epsilon\]
        Notice that
        \begin{align*}
            \abs{\sqrt{x_n}-\sqrt{x}}&=\abs{\frac{x_n-x}{\sqrt{x_n}+\sqrt{x}}}\\
            &\le \frac{\abs{x_n-x}}{\sqrt{x}}<\epsilon
        \end{align*}
    \end{proof}
\end{theorem}
...
\begin{proof}\((\Rightarrow)\)
    ...
    Let \(\epsilon>0\). Since \(x = \sup{(x_n)}\):
    \[x-\epsilon \text{ is NOT an upper boudn of } \seq{x}{n}\]
    Then 
    \[\exists N\in\N \st x_N > x-\epsilon\]
    Since \(\seq{x}{n}\) is increasing,
    \[\forall n\ge N,\, x_n>x-\epsilon\]

    On the other hand,\(x+\epsilon\) is an supper bound since x is an upper bound. Thus,
    \[x_n<x+\epsilon\]
    \[\Rightarrow \forall n\ge N, x-\epsilon<x_n<x+\epsilon\]
    \[\Rightarrow \abs{x_n - x}<\epsilon\Rightarrow\seq{x}{n}\rightarrow x\]

    \highlight{Case 2:} \(\seq{x}{n}\) is decreasing.

    Write \(y = \inf{\seq{x}{n}}\). Let \(\epsilon>0\)

    Since \(y = \inf{(x_n)}\),
    \[y+\epsilon \text{is NOT and upper bound of }\seq{x}{n}\]
    Thus
    \[\exists N \in\N \st x_n<y+\epsilon\]
    Since \(\seq{x}{n}\) is decreasing,
    \[\forall n>N, x_n<y+\epsilon\]
    
    On the other hand, \(y-\epsilon\) is a lower bound since \(y\) is a lower bound. Hence, 
    \[\forall n\in\N,  y-\epsilon<x_n\]
    \[\forall n\ge N, y-\epsilon<x_n<y+\epsilon \Rightarrow\abs{x_n-y}<\epsilon\]
...
\begin{proof}
    ...
    \end{exercise}
    When \(k>N_\epsilon,\, n_k>N_\epsilon\), thus
...
\begin{theorem}
    ...
    \begin{enumerate}
        ...
        \item \(\exists \epsilon_0 >0 \) and a subsequence \(\seq{x}{n_k}\) s.t. \[\abs{x_{n_k}-x}>\epsilon_0,\, \forall k \in \N\]
    \end{enumerate}
...
\begin{theorem}
    ...
    \begin{proof}[Proof of thm. (b) \(\Rightarrow\) (c)]
        ...
        On the other hand, there are infinitely many n with \(x^*-\epsilon<x_n\le u_n\).

        \(\text{Thus, there exists a subsequence of }u_n\text{, say }u_{n_k}\text{, satisfies}\)
        \begin{align*}
            &&x^*-\epsilon&\le u_{n_k}\\
            &\Rightarrow &x^*-\epsilon &\le \lim{u_{n_k}}=\lim{\seq{u}{n}}\\
    ...
    \begin{proof}[Proof of thm. (d) \(\Rightarrow\) (a)]
        ...
        There is no subsequence of \(x_n\) which has a limit exceeding \(x^*+\epsilon\).
        \begin{align*}
            &\Rightarrow \text{ There is only finitely many n with }x_n>x^*+\epsilon.\\
            &\Rightarrow &x^*+\epsilon&\in X\\
            &\Rightarrow &\inf{X} &\le x^*+\epsilon\\
            &\Rightarrow &\limsup{x_n}&\le x^*+\epsilon\\
            &\Rightarrow &\limsup{x_n}&\le x^*
        \end{align*}

        Next, consider \(x^*-\epsilon\)

        Then, there exists a subsequential limit of \(x_n\) which is greater or equal to \(x^*-\frac{1}{2}\epsilon\).
        
        There exists a convergent subsequence of \(\seq{x}{n}\), say \(\seq{x}{n_k}\), such that 
        \begin{align*}
            &&\lim{\seq{x}{n_k}}\ge x^*-\frac{1}{2}\epsilon\\
            &\Rightarrow&\text{ There are infinitely many n with } x^*&>x^*-\epsilon.&\\
            &\Rightarrow&\forall a\in X, x^*-\epsilon &\le a\\
            &\Rightarrow&x^*-\epsilon &\le \inf{X}\\
            &\Rightarrow&\limsup{x_n}&\ge x^*-\epsilon\\
...
\begin{definition}[\textbf{Cauchy Sequence}]
    ...
    \(\forall \epsilon>0, \exists H\in\N \st \forall n,m\in\N, n>0,m>0\),
    \[\abs{x_n-x_m}<\epsilon\]
    ...
    \begin{example}(\(\frac{1}{n}\)) is a Cauchy sequence.
        \begin{proof}
            ...
            Choose \(H=\lceil \frac{1}{\epsilon} \rceil+1\), Then

            \(\forall n,m\ge H\),
            \[\abs{\frac{1}{n}-\frac{1}{m}}\le \frac{1}{m}<\epsilon\]
        \end{proof}
    \end{example}

    \begin{example}\((-1)^n\) is not a Cauchy sequence.
        \begin{proof}
            Choose \(\epsilon_0=\frac{1}{2}, \forall H\in\N,\) choose \(n,m\ge H \st n\) is even, \(m\) is odd. Then 
...
\begin{theorem}
    ...
    \begin{proof}
        ...
        \highlight{Goal:} \[\abs{x_n-x}<\epsilon\Rightarrow\abs{x_n-x_m}<\epsilon\]
        \(\forall n,m\ge N_\epsilon,\)
        \begin{align*}
...
\begin{lemma}
    ...
    \begin{proof}
        Choose \(\epsilon=1,\exists H\ge 0 \st \forall n,m\ge H\),
        \[\abs{x_n-x_m}<1\]
...
\begin{theorem}[\textbf{Cauchy Convergence Theorem}]
    ...
    \begin{proof}[(\(\Leftarrow\))]
        ...
        \highlight{Goal:} \(\forall \epsilon>0, \exists N\in\N \st \forall n\ge N\)
        \[\abs{x_n-x}<\epsilon\]
        We will use the trick of \highlight{insert subsequence:}

        By definition of convergence, \(\forall \epsilon>0, \exists N_\epsilon\in\N \st \forall k\ge N_\epsilon\)
        \[\abs{x_{n_k}-x}<\frac{1}{2}\epsilon\]
        By definition of Cauchy sequence, \(\forall \epsilon>0, \exists H\in\N,H\ge 0 \st \forall n,m\ge H\)
        \[\abs{x_n-x_m}<\frac{1}{2}\epsilon\]
        
        Thus \(\forall k\ge \max\{H,N_\epsilon\}\),
        \begin{align*}
            \abs{x_k-x} &= \abs{(x_k-x_{n_k})+(x_{n_k}-x)}\\
            &\le \abs{x_k-x_{n_K}}+\abs{x_{n_k}-x}\text{ since }n_k\ge k\ge \max\{H, N_\epsilon\}\\
            &<\frac{1}{2}\epsilon+\frac{1}{2}\epsilon=\epsilon
        \end{align*}
    \end{proof}
\end{theorem}
...
\begin{example}
    ...
    \begin{proof}
        ...
        So we choose \(\epsilon_0=\frac{1}{2},\forall H\ge 0, \text{ choose }n=H,m=2H\), then
...
\begin{proof}
    ...
    By definition of limit, \(\forall \epsilon>0, \exists N\in\N \st \forall n>N\),
    \[L-\epsilon<\frac{x_n }{y_n }<L+\epsilon\]
    Choose \(\epsilon=\frac{1}{2}L\), we have 
...
\begin{theorem}[\textbf{Cauchy Criterion for Series}] 
    The series \(\sum x_i \) \highlight{converges} if and only if \(\forall \epsilon >0, \exists M\in\N \st \forall n,m\in\N, n>m\ge M\),
    \[\abs{x_{m+1}+x_{m+2}+\dots+x_{n-1}+x_{n}}<\epsilon\]
    Or we write it as 
    \[\abs{s_k-s_m}<\epsilon\]
...
\begin{theorem}
    ...
    \begin{proof}[Sketch of Proof](\(\Rightarrow\))
        ...
        \(\forall\delta>0,\exists N_\epsilon>0\st\)
        \[\abs{a_{N_\epsilon}-c}<\delta\]
        Note that \(a_{N_\epsilon}\in A\textbackslash\{c\}\) where \(c\) is a cluster point of \(A\).
    \end{proof}
    \newpage
...
\begin{definition}[\textbf{Delta-Epsilon Definition of Limit}]
    Let \(A\in\R\), \(c\) is a cluste point of \(A\), \(f:A\rightarrow\R\). A real number L is the \highlight{limit of f at c} if 

    \(\forall \epsilon>0, \exists\delta>0 \st \forall x\in A,\)
...
\begin{theorem}[\textbf{Uniqueness of Limit}]
    ...
    \begin{proof}
        ...
        Let \(L_1\) and \(L_2\) be limits of \(f \) at \(c \). Assume \(L_1\neq L_2 \). Choose \(\epsilon=\frac{\abs{L_1-L_2}}{2}>0\). Then,
        \[\exists \delta_1\st 0<\abs{x-c}<\delta_1\Rightarrow\abs{f(x)-L_1}<\frac{\epsilon}{2}\]
        \[\exists \delta_2\st 0<\abs{x-c}<\delta_2\Rightarrow\abs{f(x)-L_2}<\frac{\epsilon}{2}\]
        Consider \(\delta:=\min\{\delta_1,\delta_2\}\). 

        Since \(c\) is a cluster point, \(\exists x_0\in A\st\) 
        \[0<\abs{x_0-c}<\delta\]
        Since \[\abs{f(x_0)-L_1}<\frac{\epsilon}{2},\abs{f(x_0)-L_2}<\frac{\epsilon}{2}\]
        We have 
        \begin{align*}
...
\begin{theorem}
    ...
    \begin{enumerate}
        ...
        \item \(\forall\, V_\epsilon(L)\,\epsilon\text{-neighborhood of L, }\exists\, V_\delta(c)\,\delta\text{-neighborhood of }c\st\)
        \[x\in V_\delta(c)\cap (A\backslash\{c\})\Rightarrow f(x)\in V_\epsilon(c)\]
    \end{enumerate}
...
\begin{example}
    ...
    \begin{proof}
        \(\forall \epsilon>0, \exists\delta=1\st\)
...
\begin{example}
    ...
    \begin{proof}
        \(\forall \epsilon>0,\exists \delta=\epsilon\st\)
        \begin{align*}
            &x\in\neighbor{\delta }{c }\cap (A\backslash\{c \})\\
            \Rightarrow& f(x)=x\in\neighbor{\delta }{c }\cap(A\backslash\{c \})\\
            \Rightarrow&f(x)\in\neighbor{\epsilon }{c }\cap(A\backslash\{c \})\subseteq \neighbor{\epsilon }{c }
        \end{align*}
